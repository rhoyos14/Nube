...
if __name__=="__main__":
    ...
    #concatenar
    pd.concat([train_df['readmitted'], train_df.drop(['readmitted', 'readmitted'], axis=1)], axis=1).to_csv('/opt/ml/processing/output/train/train.csv', index=False, header=False)
    pd.concat([validation_df['readmitted'], validation_df.drop(['readmitted', 'readmitted'], axis=1)], axis=1).to_csv('/opt/ml/processing/output/validation/validation.csv', index=False, header=False)
